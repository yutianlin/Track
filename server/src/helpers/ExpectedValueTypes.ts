...
export class ExpectedValueTypes {
  ...
  getNotNullableStrings(): Array<string> {
    ...
    if (!strings) { throw new InvalidTypesError("strings not nullable") }
    return strings;
  }

  getNullableStrings(): Array<string> {
    const strings = this.nullable.get("strings");
    if (!strings) { throw new InvalidTypesError("strings nullable") }
    return strings;
  }

  getNotNullableBooleans(): Array<string> {
    const booleans = this.notNullable.get("booleans");
    if (!booleans) { throw new InvalidTypesError("booleans not nullable") }
    return booleans;
  }

  getNullableBooleans() {
    const booleans = this.nullable.get("booleans");
    if (!booleans) { throw new InvalidTypesError("booleans nullable") }
    return booleans;
  }

  getNotNullableDateTimes() {
    const dateTimes = this.notNullable.get("dateTimes");
    if (!dateTimes) { throw new InvalidTypesError("dateTimes not nullable") }
    return dateTimes;
  }

  getNullableDateTimes() {
    const dateTimes = this.nullable.get("dateTimes");
    if (!dateTimes) { throw new InvalidTypesError("dateTimes nullable") }
    return dateTimes;
  }

  getNotNullableNumbers() {
    const numbers = this.notNullable.get("numbers");
    if (!numbers) { throw new InvalidTypesError("numbers not nullable") }
    return numbers;
  }

  getNullableNumbers() {
    const numbers = this.nullable.get("numbers");
    if (!numbers) { throw new InvalidTypesError("numbers nullable") }
    return numbers;
  }
}
