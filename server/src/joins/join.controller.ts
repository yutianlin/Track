...
module.exports = function (app: any) {
  ...
  app.get(
    "/person_notification_messages_info/:person_id",
    async (request: any, response: any) => {
      const person_id = request.params.person_id;
      response.json(
        await joinService.getAllPersonNotificationInfoById(person_id)
      );
    }
  );

  app.get(
    "/covid_testing_centre_info/:covid_testing_centre_id",
    async (request: any, response: any) => {
      const covid_testing_centre_id = request.params.covid_testing_centre_id;
      response.json(
        await joinService.getCovidTestingCentreInfoById(covid_testing_centre_id)
      );
    }
  );

  app.get(
    "/scheduled_class_info/:scheduled_class_id",
    async (request: any, response: any) => {
      const scheduled_class_id = request.params.scheduled_class_id;
      response.json(
        await joinService.getScheduledClassDayInfo(scheduled_class_id)
      );
    }
  );

  app.get(
    "/peron_entrance_room_building_time",
    jsonParser,
    async (request: any, response: any) => {
      const requestBody = request.body.data;
      response.json(
        await joinService.getPersonEntranceRoomBuildingTimeInfo(requestBody)
      );
    }
  );
  ...
  app.get(
    ...
    async (request: any, response: any) => {
      const person_id = request.params.person_id;
      response.json(
        await joinService.getAllUnreadNotificationsByPersonId(person_id)
      );
    }
  );
  ...
  app.get(
    "/scheduled_class_info/person_id/:person_id",
    async (request: any, response: any) => {
      const person_id = request.params.person_id;
      response.json(
        await joinService.getScheduledClassDayInfoByPersonId(person_id)
      );
    }
  );
